...
```
Which team has the most players in the top 50? 
...
wbb_df |> group_by(Cl) |>
  summarize(Name)
```
Might bring in another data set of iowa stats and SC stats
...
```{r}
iowa_df |> arrange(desc(MIN)) |>
  slice(1:10) |>
  summarize(points_avg = mean(PTS))
```
This is the points average for the top 10 most time played players on the Iowa team

```{r}
sc_df |> arrange(desc(MIN)) |>
  slice(1:10) |>
  summarize(points_avg = mean(PTS))
```
...
sc_df |> arrange(desc(MIN)) |>
  ...
  summarize(fg_made = mean(`FG%`))
```
As we can see by this, Iowa has a higher percent of field goals made still by the top 10 with the most minutes, by around 2%.
